...
public class InfoResourceTest extends JerseySpringTest {
	...
	public void sameService() {
		...
		assertEquals(String.valueOf(service.hashCode()), target("info/service-hash-code").request().get(String.class));
	}

	@Test
	public void resourceIsPrototype() {
		// 资源类通过 @Named 配置为原型类型
		assertNotEquals(target("info/hash-code").request().get(String.class), target("info/hash-code").request().get(String.class));
	}
