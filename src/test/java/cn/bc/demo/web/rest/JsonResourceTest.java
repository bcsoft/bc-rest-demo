...
public class JsonResourceTest extends JerseySpringTest {
	@Test
	public void getJsonObject() {
		Response res = target("json/1").request().get();
	...
	public void getJsonArray() {
		Response res = target("json").request().get();
		assertEquals(OK.getStatusCode(), res.getStatus());
		assertEquals(MediaType.APPLICATION_JSON_TYPE, res.getMediaType());
		// or assertEquals([{"id":1}], res.readEntity(String.class));
	...
	public void postJsonObject() {
		JsonObject json = Json.createObjectBuilder()
				...
				.add("num", 1.1)
				.build();
		Response res = target("json/post-object").request().post(Entity.json(json));
		assertEquals(OK.getStatusCode(), res.getStatus());
		assertEquals(MediaType.APPLICATION_JSON_TYPE, res.getMediaType());
	...
	public void postJsonArray() {
		JsonArray jsons = Json.createArrayBuilder()
				.add(Json.createObjectBuilder()
						...
						.add("num", 1.1))
				.build();
		Response res = target("json/post-array").request().post(Entity.json(jsons));
